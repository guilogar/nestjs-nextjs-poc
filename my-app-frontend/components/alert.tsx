...
import { alertService, AlertType } from '../services/generic';
...
const Alert: NextPage<AlertData> = ({ id, fade }: AlertData) => {
    ...
    const subscription = alertService.onAlert(id)
      .subscribe(alert => {
        ...
        if (!alert.message) {
          ...
          // add alert to array
          setAlerts((alerts) => ([...alerts, alert]) as any);
  ...
  function removeAlert(alert: any) {
    if (fade) {
      ...
      setAlerts(
        (alerts) => alerts.map(x => x === alert ? alertWithFade : x) as any
      );
